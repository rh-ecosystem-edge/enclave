...
#   RGW_PORT       - RadosGW port (default: 8080)
...
LOOP_DIR="${LOOP_DIR:-/disk1/ceph-loops}"
...
if command -v cephadm &>/dev/null; then
    ...
    python3 cephadm.py add-repo --release "$CEPH_RELEASE"
...
# Deploy ceph-exporter for monitoring endpoint (port 9283, required by ODF)
info "Deploying ceph-exporter for monitoring metrics..."
cephadm shell -- ceph orch apply ceph-exporter 2>/dev/null || true
success "ceph-exporter deployed (port 9283)"
...
")
    ...
    # Create a dedicated CSI user for RBD
    cephadm shell -- ceph auth get-or-create client.csi-rbd-node \
        mon 'profile rbd' \
        osd "profile rbd pool=${RBD_POOL}" 2>/dev/null || true
    CSI_RBD_NODE_KEY=$(cephadm shell -- ceph auth get-key client.csi-rbd-node 2>/dev/null)

    cephadm shell -- ceph auth get-or-create client.csi-rbd-provisioner \
        mon 'profile rbd' \
        mgr 'allow rw' \
        osd "profile rbd pool=${RBD_POOL}" 2>/dev/null || true
    CSI_RBD_PROVISIONER_KEY=$(cephadm shell -- ceph auth get-key client.csi-rbd-provisioner 2>/dev/null)
...
data = [
    ...
    {'name': 'rook-csi-rbd-node', 'kind': 'Secret', 'data': {'userID': 'csi-rbd-node', 'userKey': '${CSI_RBD_NODE_KEY}'}},
    {'name': 'rook-csi-rbd-provisioner', 'kind': 'Secret', 'data': {'userID': 'csi-rbd-provisioner', 'userKey': '${CSI_RBD_PROVISIONER_KEY}'}},
    ...
    {'name': 'ceph-rgw', 'kind': 'StorageClass', 'data': {'endpoint': 'http://${CEPH_HOST_IP}:${RGW_PORT}', 'poolPrefix': 'default'}},
