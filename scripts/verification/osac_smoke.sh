...
# TENANT_SC_ORIG holds the StorageClass's original osac.openshift.io/tenant label
# value so cleanup restores it (empty means the label was absent -> remove it).
TENANT_NAME=""
TENANT_SC=""
TENANT_SC_ORIG=""
cleanup() {
    ...
    if [ -n "${TENANT_SC}" ]; then
        if [ -n "${TENANT_SC_ORIG}" ]; then
            oc label storageclass "${TENANT_SC}" "osac.openshift.io/tenant=${TENANT_SC_ORIG}" --overwrite >/dev/null 2>&1 || true
        else
...
# Fail if any core deployment's current revision is not fully rolled out. Uses
# oc rollout status rather than comparing availableReplicas to spec.replicas: a
# stalled rolling update can keep the old replica available while the new one is
# not ready, which the replica count alone would pass.
assert_deployments_available() {
    ...
    for dep in ${CORE_DEPLOYMENTS}; do
        ...
        if oc -n "${NS}" rollout status "deployment/${dep}" --timeout=5m; then
            info "deployment ${dep} rolled out"
        else
            error "core deployment ${dep} did not complete rollout"
            rc=1
        fi
    done
    return "${rc}"
...
run_tenant_test() {
    ...
    # Remember any existing tenant label so cleanup restores it instead of
    # clobbering a real tenant association on the borrowed StorageClass.
    TENANT_SC_ORIG="$(oc get storageclass "${TENANT_SC}" -o jsonpath='{.metadata.labels.osac\.openshift\.io/tenant}' 2>/dev/null)"
...
YAML
    ...
    if [ -n "${TENANT_SC_ORIG}" ]; then
        oc label storageclass "${TENANT_SC}" "osac.openshift.io/tenant=${TENANT_SC_ORIG}" --overwrite 2>/dev/null || true
    else
