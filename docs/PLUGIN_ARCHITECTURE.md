...
When a plugin declares a `helm:` block, the Helm CLI is downloaded so steps 6 and 10 can run. It is downloaded alongside the other control binaries: during the prepare phase for day-0 plugins, and by the deploy-plugin playbook for day-2 plugins (the download is idempotent and checksum-validated, so it is a no-op when Helm is already present). Either way plugins that use Helm work whether they are deployed at install time or on day-2. The CLI is not installed otherwise.
